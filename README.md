...
This template uses Vite for building the JavaScript/React components.
...
Shared async chunks for code that repeats across multiple async chunks is already supported through our custom Vite configuration optimizations. You can leverage it by manually the path of `{{cookiecutter.project_shortname}}-shared.js` to `_js_dist` inside `{{cookiecutter.project_shortname}}/__init__.py` (as well as the associated external URL).

## More Resources

- Found a bug or have a feature request? [Create an issue](https://github.com/HogaStack/dash-component-vite-boilerplate/issues/new)
- Watch the [component boilerplate repository](https://github.com/HogaStack/dash-component-vite-boilerplate) to stay informed of changes to our components.
